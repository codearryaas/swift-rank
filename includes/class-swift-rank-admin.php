...
class Swift_Rank_Admin {
		...
		<div class="swift-rank-field-wrapper">
			<div class="swift-rank-input-group">
				<input
					...
					<?php echo $args['required'] && ! $has_variable ? 'required' : ''; ?>
				/>
	...
	public function organization_type_field_callback() {
		...
		<p class="description"><?php esc_html_e( 'Select the type of entity to represent. Choose "Local Business" for businesses with physical locations.', 'swift-rank' ); ?></p>
		<?php
	}
	...
	public function organization_industry_field_callback() {
		...
		<select name="swift_rank_settings[organization_industry]" id="swift_rank_settings_organization_industry">
			...
			<?php foreach ( $industries as $key => $label ) : ?>
				<option value="<?php echo esc_attr( $key ); ?>" <?php selected( $industry, $key ); ?>><?php echo esc_html( $label ); ?>
				</option>
			<?php endforeach; ?>
		</select>
		<p class="description"><?php esc_html_e( 'Select the industry that best describes your organization.', 'swift-rank' ); ?>
		</p>
		<?php
	}
	...
	public function organization_price_range_field_callback() {
		...
		<p class="description" style="margin-top: 4px; padding-left: 12px; border-left: 3px solid #ddd;">
			<strong><?php esc_html_e( 'Symbol format:', 'swift-rank' ); ?></strong> $ (inexpensive), $$ (moderate), $$$ (expensive), $$$$ (very expensive)<br>
	...
	public function organization_hours_field_callback() {
		...
		<div class="swift-rank-opening-hours-container">
			...
			foreach ( $days_of_week as $day ) :
				...
				?>
				<div class="swift-rank-hours-row <?php echo $is_closed ? 'closed' : ''; ?>" data-day="<?php echo esc_attr( $day ); ?>">
					<label class="day-label"><?php echo esc_html( $day ); ?></label>
					<label style="display: flex; align-items: center; gap: 6px; font-size: 12px;">
						<input type="checkbox"
							name="swift_rank_settings[organization_hours][<?php echo esc_attr( $day ); ?>][closed]"
							value="1"
							class="swift-rank-closed-checkbox"
					...
					<div>
						...
						<input type="time"
							name="swift_rank_settings[organization_hours][<?php echo esc_attr( $day ); ?>][opens]"
					...
					<div>
						...
						<input type="time"
							name="swift_rank_settings[organization_hours][<?php echo esc_attr( $day ); ?>][closes]"
	...
	public function organization_social_field_callback() {
		...
		<div class="swift-rank-repeater swift-rank-settings-repeater" data-repeater-path="organization_social">
						...
						<div class="swift-rank-repeater-item-content">
							...
							<div class="swift-rank-input-group">
								<input
									...
									class="regular-text swift-rank-variable-field"
			...
			<button type="button" class="button button-secondary swift-rank-add-repeater-item" data-repeater-id="swift_rank_repeater_organization_social">
						...
						<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 20px; margin-top: 20px;">
							<div style="background: #f0f6fc; border-left: 4px solid #2271b1; padding: 16px;">
								<h3 style="margin-top: 0;">
									<span class="dashicons dashicons-google" style="font-size: 20px; vertical-align: middle;"></span>
									<?php esc_html_e( 'Google Rich Results Test', 'swift-rank' ); ?>
								</h3>
								<p><?php esc_html_e( 'Test how Google sees your structured data and check for rich results eligibility.', 'swift-rank' ); ?></p>
							...
							<div style="background: #fff8e5; border-left: 4px solid #dba617; padding: 16px;">
								<h3 style="margin-top: 0;">
									<span class="dashicons dashicons-yes-alt" style="font-size: 20px; vertical-align: middle;"></span>
									<?php esc_html_e( 'Schema.org Validator', 'swift-rank' ); ?>
								</h3>
								<p><?php esc_html_e( 'Validate your schema markup against official Schema.org standards.', 'swift-rank' ); ?></p>
				...
				<div class="postbox" style="margin-top: 20px;">
					<div class="inside">
						...
						<ol style="line-height: 1.8;">
							<li><strong><?php esc_html_e( 'View Page Source', 'swift-rank' ); ?></strong> - <?php esc_html_e( 'Visit your homepage and press Ctrl+U (Windows) or Cmd+U (Mac) to view the source code.', 'swift-rank' ); ?></li>
							<li><strong><?php esc_html_e( 'Search for Schema', 'swift-rank' ); ?></strong> - <?php esc_html_e( 'Look for "Swift Rank" comment or search for "application/ld+json" to find the schema output.', 'swift-rank' ); ?></li>
							<li><strong><?php esc_html_e( 'Verify Data', 'swift-rank' ); ?></strong> - <?php esc_html_e( 'Check that all your organization information appears correctly in the JSON-LD script.', 'swift-rank' ); ?></li>
						</ol>
						...
						<table class="widefat striped">
							...
							<tbody>
								<tr>
									...
									<td><?php esc_html_e( 'Fill in all required fields marked with * in the Knowledge Graph settings.', 'swift-rank' ); ?></td>
								</tr>
								<tr>
									<td><?php esc_html_e( 'Invalid URL format', 'swift-rank' ); ?></td>
									<td><?php esc_html_e( 'Ensure all URLs start with https:// or http:// and are properly formatted.', 'swift-rank' ); ?></td>
								</tr>
								<tr>
									<td><?php esc_html_e( 'Invalid phone number', 'swift-rank' ); ?></td>
									<td><?php esc_html_e( 'Use international format with country code (e.g., +1-555-123-4567).', 'swift-rank' ); ?></td>
								</tr>
								<tr>
									<td><?php esc_html_e( 'Logo too large', 'swift-rank' ); ?></td>
									<td><?php esc_html_e( 'Google recommends logos with max height of 60px for best results.', 'swift-rank' ); ?></td>
								</tr>
							</tbody>
						</table>
					</div>
	...
	private function render_help_tab() {
		...
		<div class="swift-rank-help-section" style="max-width: 1200px;">
			...
			<p><?php esc_html_e( 'Learn how to configure your Knowledge Graph schema for optimal search engine visibility.', 'swift-rank' ); ?></p>

			<div class="postbox" style="margin-top: 20px;">
				<div class="inside">
					<h3 style="margin-top: 0;"><?php esc_html_e( 'What is Knowledge Graph?', 'swift-rank' ); ?></h3>
					<p><?php esc_html_e( 'The Knowledge Graph is structured data that tells search engines important information about your organization, business, or person. This helps search engines display rich information about you in search results.', 'swift-rank' ); ?></p>

					<h4><?php esc_html_e( 'Schema Types', 'swift-rank' ); ?></h4>
					<ul style="line-height: 1.8;">
						<li><strong><?php esc_html_e( 'Organization', 'swift-rank' ); ?></strong> - <?php esc_html_e( 'For companies, corporations, and general businesses without physical locations.', 'swift-rank' ); ?></li>
						<li><strong><?php esc_html_e( 'LocalBusiness', 'swift-rank' ); ?></strong> - <?php esc_html_e( 'For businesses with physical locations, operating hours, and price ranges (e.g., restaurants, stores, offices).', 'swift-rank' ); ?></li>
						<li><strong><?php esc_html_e( 'Person', 'swift-rank' ); ?></strong> - <?php esc_html_e( 'For personal websites, portfolios, or individual professionals (e.g., freelancers, authors, consultants).', 'swift-rank' ); ?></li>
					</ul>
				</div>
			</div>
					...
					<table class="widefat striped">
						...
						<tbody>
							<tr>
								...
								<td><?php esc_html_e( 'The legal name of your organization or your personal name. Use variables like {site_name} for dynamic content.', 'swift-rank' ); ?></td>
								<td><span style="color: #d63638;">✓</span></td>
							</tr>
							<tr>
								<td><strong><?php esc_html_e( 'Logo / Image', 'swift-rank' ); ?></strong></td>
								<td><?php esc_html_e( 'URL to your logo (for organizations) or profile photo (for persons). Recommended: square or 16:9 ratio, max 60px height, PNG format.', 'swift-rank' ); ?></td>
								<td><?php esc_html_e( 'Recommended', 'swift-rank' ); ?></td>
							</tr>
							<tr>
								<td><strong><?php esc_html_e( 'Contact Information', 'swift-rank' ); ?></strong></td>
								<td><?php esc_html_e( 'Phone, email, and fax numbers help users contact you directly from search results. Use international format for phone numbers.', 'swift-rank' ); ?></td>
								<td><?php esc_html_e( 'Optional', 'swift-rank' ); ?></td>
							</tr>
							<tr>
								<td><strong><?php esc_html_e( 'Address', 'swift-rank' ); ?></strong></td>
								<td><?php esc_html_e( 'Full postal address including street, city, state, postal code, and country. Important for local businesses.', 'swift-rank' ); ?></td>
								<td><?php esc_html_e( 'Optional', 'swift-rank' ); ?></td>
							</tr>
							<tr>
								<td><strong><?php esc_html_e( 'Social Media Profiles', 'swift-rank' ); ?></strong></td>
								<td><?php esc_html_e( 'Links to your official social media accounts (Facebook, Twitter, LinkedIn, Instagram, etc.). Helps verify your identity.', 'swift-rank' ); ?></td>
								<td><?php esc_html_e( 'Recommended', 'swift-rank' ); ?></td>
							</tr>
							<tr>
								<td><strong><?php esc_html_e( 'Opening Hours', 'swift-rank' ); ?></strong></td>
								<td><?php esc_html_e( 'Business hours for each day of the week. Only for LocalBusiness schema type.', 'swift-rank' ); ?></td>
								<td><?php esc_html_e( 'LocalBusiness only', 'swift-rank' ); ?></td>
							</tr>
							<tr>
								<td><strong><?php esc_html_e( 'Price Range', 'swift-rank' ); ?></strong></td>
								<td><?php esc_html_e( 'Indicates pricing level using $, $$, $$$, or $$$$ symbols. Only for LocalBusiness schema type.', 'swift-rank' ); ?></td>
								<td><?php esc_html_e( 'LocalBusiness only', 'swift-rank' ); ?></td>
							</tr>
						</tbody>
					</table>
			...
			<p><?php esc_html_e( 'You can use these variables in your schema fields. They will be replaced with actual values when the schema is output.', 'swift-rank' ); ?></p>
						...
						<table class="widefat striped">
							...
							<tbody>
								<tr>
									...
									<td><?php esc_html_e( 'Get any WordPress option value (replace "option_name" with actual option name)', 'swift-rank' ); ?></td>
								</tr>
							</tbody>
						</table>
					</div>
			...
			<div class="postbox" style="margin-top: 20px;">
				<div class="inside">
					...
					<ol style="line-height: 1.8;">
						<li><strong><?php esc_html_e( 'Click "Insert Variable" button', 'swift-rank' ); ?></strong> - <?php esc_html_e( 'Look for the button next to input fields in the settings.', 'swift-rank' ); ?></li>
						<li><strong><?php esc_html_e( 'Select a variable', 'swift-rank' ); ?></strong> - <?php esc_html_e( 'Choose from the dropdown menu of available variables.', 'swift-rank' ); ?></li>
						<li><strong><?php esc_html_e( 'Variable is inserted', 'swift-rank' ); ?></strong> - <?php esc_html_e( 'The variable will be added to the field at cursor position.', 'swift-rank' ); ?></li>
						<li><strong><?php esc_html_e( 'Mix with text', 'swift-rank' ); ?></strong> - <?php esc_html_e( 'You can combine variables with static text as needed.', 'swift-rank' ); ?></li>
					</ol>
					...
					<div style="background: #f0f6fc; border-left: 4px solid #2271b1; padding: 12px; margin-top: 16px;">
						...
						<p><strong><?php esc_html_e( 'Organization Name:', 'swift-rank' ); ?></strong> <code>{site_name}</code></p>
						<p><strong><?php esc_html_e( 'Custom Option:', 'swift-rank' ); ?></strong> <code>{option:blogdescription}</code></p>
						<p style="margin-bottom: 0;"><strong><?php esc_html_e( 'Mixed:', 'swift-rank' ); ?></strong> <code>Contact {site_name} at {option:admin_email}</code></p>
					</div>
				</div>
			</div>
			...
			<div class="postbox" style="margin-top: 20px;">
				<div class="inside">
					...
					<p><?php esc_html_e( 'After configuring your schema settings, validate it with these tools:', 'swift-rank' ); ?></p>
					<ul style="line-height: 1.8;">
						<li>
							<strong><a href="https://search.google.com/test/rich-results" target="_blank" rel="noopener"><?php esc_html_e( 'Google Rich Results Test', 'swift-rank' ); ?></a></strong><br>
							<span class="description"><?php esc_html_e( 'Test how Google sees your structured data and check for errors.', 'swift-rank' ); ?></span>
						</li>
						<li style="margin-top: 12px;">
							<strong><a href="https://validator.schema.org/" target="_blank" rel="noopener"><?php esc_html_e( 'Schema.org Validator', 'swift-rank' ); ?></a></strong><br>
							<span class="description"><?php esc_html_e( 'Validate your schema markup against Schema.org standards.', 'swift-rank' ); ?></span>
						</li>
						<li style="margin-top: 12px;">
							<strong><?php esc_html_e( 'View Page Source', 'swift-rank' ); ?></strong><br>
							<span class="description"><?php esc_html_e( 'Visit your homepage and view source (Ctrl+U / Cmd+U). Look for JSON-LD script tag in the <head> section.', 'swift-rank' ); ?></span>
						</li>
					</ul>
				</div>
			</div>
	...
	public function enqueue_admin_assets( $hook ) {
		...
		wp_add_inline_script(
			'swift-rank-admin',
			'var schemaVariables = ' . wp_json_encode( $this->get_schema_variables() ) . ';',
			'before'
		);
	}
